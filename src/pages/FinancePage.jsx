                    ...
                    {group.transactions.map((transaction) => {
                      ...
                      return (
                        <div key={transaction.id} className="grid gap-3 px-5 py-4 md:grid-cols-[1fr_auto] md:items-center">
                          <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="truncate text-sm font-black text-slate-800">{transaction.note}</p><span className={`rounded px-2 py-0.5 text-[9px] font-black uppercase tracking-wide ${transaction.category === 'rental' ? 'bg-cyan-50 text-cyan-700' : 'bg-slate-100 text-slate-500'}`}>{transaction.category === 'rental' ? transaction.rentalCode || 'Penyewaan' : 'Manual'}</span></div><p className="mt-1 text-xs font-semibold text-slate-500">{transaction.customerName ? `Tamu ${transaction.customerName}` : income ? 'Pendapatan lain' : 'Pengeluaran operasional'}{transaction.costAmount ? ` / Modal ${formatCurrency(transaction.costAmount)}` : ''}</p></div>
